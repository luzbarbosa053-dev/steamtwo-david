...
function Header({ view, onNavigate, search, setSearch, favoriteCount }) { return <header className={`site-header ${view === "home" ? "site-header-overlay" : ""}`}><button className="brand" onClick={() => onNavigate("home")} aria-label="SteamTwo, início"><span>Steam</span><b>Two</b></button><nav aria-label="Navegação principal"><button className={view === "home" ? "active" : ""} onClick={() => onNavigate("home")}>Início</button><button className={view === "catalog" ? "active" : ""} onClick={() => onNavigate("catalog")}>Catálogo</button><button className={view === "rankings" ? "active" : ""} onClick={() => onNavigate("rankings")}>Rankings</button></nav><button className="header-favorites" onClick={() => onNavigate("catalog-favorites")} aria-label="Filtrar favoritos"><Star size={18} weight={favoriteCount ? "fill" : "regular"} /> {favoriteCount}</button><label className="search-box"><MagnifyingGlass size={20} /><input value={search} onChange={(event) => { const value = event.target.value; setSearch(value); if (value && view !== "catalog") onNavigate("catalog"); }} placeholder="Buscar jogos" aria-label="Buscar jogos" /></label></header>; }
...
function Detail({ game, favorite, onToggleFavorite, onBack, onMethodology }) { return <section className="detail-page"><button className="back-link" onClick={onBack}><ArrowRight size={17} /> Voltar</button><div className="detail-hero" style={{ "--detail-image": `url(${game.heroUrl})` }}><div><span className="eyebrow">DETALHE DO JOGO</span><h1>{game.shortTitle}</h1><div className="detail-tags">{game.genres.map((genre) => <span key={genre}>{genre}</span>)}</div><p>{game.summary}</p><div className="detail-actions"><a className="primary-button" href={storeUrlForGame(game)} target="_blank" rel="noreferrer">Abrir na loja <ArrowSquareOut size={17} /></a><button className="list-button" onClick={() => onToggleFavorite(game)}><Star size={19} weight={favorite ? "fill" : "regular"} /> {favorite ? "Remover dos favoritos" : "Favoritar jogo"}</button></div></div><div className="detail-score"><span>ÍNDICE STEAMTWO</span><Score value={game.score} large /><small>/ 100</small><button onClick={onMethodology}>Como calculamos <Info size={14} /></button></div></div><div className="detail-columns"><article><h2>Sobre este jogo</h2><p>{game.summary} O índice combina tração, qualidade e engajamento para oferecer uma leitura comparável entre os títulos.</p></article><aside><h2>Fontes</h2><div className="source-pills">{game.stores.map((store) => <span key={store}><Check size={14} /> {storeLabel(store)}</span>)}<span><Check size={14} /> IGDB</span></div><small>Última atualização: 24 de ago. de 2026</small></aside></div></section>; }
...
export function App() {
  ...
  return <div className="app-shell"><Header view={selected ? "" : view} onNavigate={navigate} search={search} setSearch={setSearch} favoriteCount={favoriteSlugs.size} />{loading && <div className="loading-bar" aria-label="Carregando dados" />}<main>{selected ? <Detail game={selected} favorite={favoriteSlugs.has(selected.slug)} onToggleFavorite={toggleFavorite} onBack={() => navigate("home")} onMethodology={() => setMethodology(true)} /> : view === "catalog" ? <Catalog data={data} query={search} onDetails={details} favorites={favoriteSlugs} onToggleFavorite={toggleFavorite} favoriteOnly={favoriteOnly} onShowFavorites={() => navigate(favoriteOnly ? "catalog" : "catalog-favorites")} /> : view === "rankings" ? <Rankings data={data} onDetails={details} /> : <Home data={data} onDetails={details} onMethodology={() => setMethodology(true)} />}</main><footer><span>SteamTwo</span><span>Dados públicos, rankings transparentes.</span><button onClick={() => setMethodology(true)}>Metodologia</button>{stale && <small>Últimos dados válidos · {formatDate(data.updatedAt)}</small>}</footer>{methodology && <Methodology onClose={() => setMethodology(false)} />}</div>;
}
