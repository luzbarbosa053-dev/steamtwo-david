...
function ComparePanel({ items, onRemove, loading }) { return <aside className="compare-panel"><div><strong>Comparador</strong><span>{items.length}/3 jogos</span></div>{loading ? <p>Comparando…</p> : items.length < 2 ? <p>Selecione mais um jogo para comparar.</p> : <div className="compare-grid">{items.map((game) => <div key={game.slug}><button onClick={() => onRemove(game.slug)} aria-label={`Remover ${game.shortTitle}`}>×</button><b>{game.shortTitle}</b><span>Índice {formatScore(game.score)}</span><span>{game.stores.map(storeLabel).join(" + ")}</span><Trend value={game.trend} /></div>)}</div>}</aside>; }
